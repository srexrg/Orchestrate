...
import {ApiError, CreateVenueInput, UpdateVenueInput} from '@orchestrate/shared';
...
export const getAvailableVenues = async (capacity: number) => {
    try {
        // In microservices, we only check venue capacity here
        // Event service should handle date availability checks
        const venues = await prisma.venue.findMany({
            where: {
                capacity: {
                    gte: capacity
